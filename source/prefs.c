/* prefs.c -- persistent key/value store backing the runner's preference store
 *
...
static void set_raw(const char *key, const char *val) {
  ...
  dirty = 1;
  // Do NOT flush here -- SD card writes on every save call cause frame stutters.
  // prefs_flush() is called once at clean exit in main.c.
}
...
void prefs_delete(const char *key) {
  for (int i = 0; i < entry_count; i++) {
    if (!strcmp(entries[i].key, key)) {
      ...
      dirty = 1;
      // prefs_flush() deferred to exit -- no immediate SD write
      return;
    }
  }
}
