/* libc_shim.h -- bionic-compatible libc wrappers for libyoyo.so
 *
...
// networking (the runner's HTTP layer) -- stubbed; the port has no network access
...
extern const char *ctype_fake_ptr;
void ctype_init(void);

size_t __strlcat_chk_fake(char *dst, const char *src, size_t siz, size_t dstlen);
size_t __strlcpy_chk_fake(char *dst, const char *src, size_t siz, size_t dstlen);
int getuid_fake(void);
const char *getprogname_fake(void);
void _exit_fake(int status);
int fcntl_fake(int fd, int cmd, ...);
int raise_fake(int sig);
int getpagesize_fake(void);
int getppid_fake(void);
int getpgid_fake(int pid);
int getpriority_fake(int which, int who);
int sigpending_fake(void *set);
...
int fstatfs_fake(int fd, void *buf);
int fstatvfs_fake(int fd, void *buf);
int sigaction_fake(int sig, const void *act, void *oact);
int signal_fake(int sig, void *func);
...
int getnameinfo_fake(const void *addr, unsigned addrlen, char *host, unsigned hostlen, char *serv, unsigned servlen, int flags);
int getpeername_fake(int fd, void *addr, unsigned *addrlen);
char *inet_ntoa_fake(uint32_t in_addr);
unsigned int inet_addr_fake(const char *cp);
unsigned int if_nametoindex_fake(const char *ifname);

int __android_log_vprint_fake(int prio, const char *tag, const char *fmt, va_list ap);

// Tell the shim which directory the NRO was launched from, so it can undo the
// runner's occasional doubled-up paths without a hardcoded folder name. Call
// once at startup, after chdir().
