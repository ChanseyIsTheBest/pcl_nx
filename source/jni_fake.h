/* jni_fake.h -- fake JNI environment for the GameMaker Android runner
 *
 * libyoyo.so talks to the Android framework exclusively through JNI: it
 * resolves Java classes via a cached ClassLoader and calls methods on
 * com.yoyogames.runner.RunnerJNILib and the game's RunnerActivity. We provide a
 * functional JNIEnv/JavaVM so those calls land on native C implementations, and
 * drive the runner's own exported entry points (Startup/Process/TouchEvent/...)
...
extern void *g_onGPDeviceAdded;
extern void *g_onGamepadChange;
...
// The video helper (the runner's video helper) reports playback events back to the
...
// On-screen keyboard (IME): the runner opens the soft keyboard and feeds typed text
...
// the runner's edit box (the runner's edit-box helper): swkbd-backed text entry, reported
