/* libc_shim.c -- bionic-compatible libc wrappers for libyoyo.so
 *
...
static char s_base_dev[512];   // defined with the base-dir helpers below

// GameMaker's Android startup temporarily clears the newlib SD devoptab slot
// while it probes Java storage. Existing FILE handles still reference that
// slot, so the next seek dereferences NULL inside _lseek_r. Preserve the
// original libnx registration and restore it before stdio reaches newlib.
// Guarded: sdmc, plus whatever device the game folder is on (e.g. a USB drive).
static int s_guard_dev[2] = { -1, -1 };
static const devoptab_t *s_guard_tab[2] = { NULL, NULL };

static void guard_capture(int slot, const char *devpath) {
  int dev = FindDevice(devpath);
  if (dev < 0 || dev >= STD_MAX || !devoptab_list[dev]) return;
  if (slot == 1 && dev == s_guard_dev[0]) return;   // same device as sdmc
  s_guard_dev[slot] = dev;
  s_guard_tab[slot] = devoptab_list[dev];
  debugPrintf("fsdev_guard: captured %s slot %d at %p\n", devpath, dev, (void *)s_guard_tab[slot]);
}

void fsdev_guard_init(void) {
  guard_capture(0, "sdmc:/");
  // device of the base directory, if it is not sdmc
  char dev[40] = { 0 };
  const char *base = s_base_dev;
  const char *colon = strchr(base, ':');
  if (colon && (size_t)(colon - base) + 2 < sizeof(dev)) {
    memcpy(dev, base, (size_t)(colon - base) + 1);
    strcat(dev, "/");
    guard_capture(1, dev);
  }
  if (s_guard_dev[0] < 0) debugPrintf("fsdev_guard: could not capture sdmc devoptab\n");
}

void fsdev_guard_restore(void) {
  for (int i = 0; i < 2; i++) {
    const int d = s_guard_dev[i];
    if (d >= 0 && d < STD_MAX && s_guard_tab[i] && devoptab_list[d] == NULL) {
      devoptab_list[d] = s_guard_tab[i];
      debugPrintf("fsdev_guard: restored slot %d\n", d);
    }
  }
}
...
int __system_property_get_fake(const char *name, char *value) {
  ...
  if (name) {
    ...
    // Oboe reads these. Returning nothing made getSdkVersion() come back 0, so
    // QuirksManager treated us as pre-Android-L: it refused float streams and
    // forced an I16 conversion layer, and skipped performance-mode setup as
    // "pre-N-MR1". Report the same API level we hand to Startup(), and identify
    // as hardware no vendor quirk list has ever heard of.
    if (!strcmp(name, "ro.build.version.sdk")) { snprintf(value, 92, "28"); return (int)strlen(value); }
    if (!strcmp(name, "ro.build.version.release")) { snprintf(value, 92, "9"); return (int)strlen(value); }
    if (!strcmp(name, "ro.product.manufacturer") ||
        !strcmp(name, "ro.soc.manufacturer")) { snprintf(value, 92, "Nintendo"); return (int)strlen(value); }
    if (!strcmp(name, "ro.product.model") ||
        !strcmp(name, "ro.product.device") ||
        !strcmp(name, "ro.product.name")) { snprintf(value, 92, "Switch"); return (int)strlen(value); }
    if (!strcmp(name, "ro.hardware")) { snprintf(value, 92, "tegra"); return (int)strlen(value); }
...
long syscall_fake(long number, ...) {
  switch (number) {
    ...
    case ARM64_SYS_GETRANDOM: {
      // LibreSSL's getentropy() tries this first; without it, it falls back to
      // hashing mmap/statfs/getrusage noise. Serve it from libnx's CSPRNG.
      va_list va;
      va_start(va, number);
      ...
      if (len > 256) len = 256;              // getentropy never asks for more
      randomGet(buf, len);
      return (long)len;
    }
  }
...
// The folder the NRO was launched from, as a POSIX path with a trailing slash
// (e.g. "/switch/pcl_nx/"). Discovered at startup rather than hardcoded, so
// the game runs from any directory under /switch -- or anywhere else on the
// card. Set by libc_shim_set_base_dir(); until then the duplicate-prefix strip
// below is simply skipped, which is harmless.
static char s_base_dir[512] = { 0 };
static size_t s_base_len = 0;
static char s_base_dev[512] = "sdmc:/";   // full base path incl. device

void libc_shim_set_base_dir(const char *dir) {
  if (!dir || !*dir) return;
  snprintf(s_base_dev, sizeof(s_base_dev), "%s", dir);

  // Store as a plain POSIX path with exactly one trailing slash: the runner
  // builds its own paths by concatenation and never includes the device prefix.
  // Any device ("sdmc:", "ums0:", ...) is stripped, not just sdmc.
  const char *p = dir;
  {
    const char *colon = strchr(p, ':');
    const char *slash = strchr(p, '/');
    if (colon && (!slash || colon < slash)) p = colon + 1;
  }

  size_t n = strlen(p);
  while (n > 0 && p[n - 1] == '/') n--;            // drop existing trailing '/'
  if (n == 0 || n + 2 > sizeof(s_base_dir)) return;

  memcpy(s_base_dir, p, n);
  s_base_dir[n] = '/';
  s_base_dir[n + 1] = '\0';
  s_base_len = n + 1;
  debugPrintf("libc_shim: base dir \"%s\"\n", s_base_dir);
}

static int is_devch(char c) {
...
static const char *sanitize_path(const char *path) {
  ...
  // Keep only the LAST "<device>:/" prefix. The runner sometimes glues a full
  // device path onto another one ("sdmc:/a/sdmc:/a/file"). Works for any
  // device name, so the game can live on sdmc, a USB drive, etc.
  for (const char *p = orig; *p; p++) {
    if (p[0] == ':' && p[1] == '/') {
      const char *s = p;
      while (s > orig && is_devch(s[-1])) s--;
      // boundary: start of string, a '/', or another device's ':' ("sdmc:sdmc:/")
      if (s < p && (s == orig || s[-1] == '/' || s[-1] == ':')) path = s;
    }
  }

  // Collapse a doubled base directory at the front, e.g.
  //   /switch/pcl_nx//switch/pcl_nx/save.dat  ->  /switch/pcl_nx/save.dat
  // Only when the base appears at the very start AND again immediately after,
  // so ordinary paths that merely contain the folder name are left alone.
  if (s_base_len > 1) {
    for (int guard = 0; guard < 4; guard++) {
      const char *posix = path;
      const char *colon = strchr(posix, ':');
      const char *slash = strchr(posix, '/');
      if (colon && (!slash || colon < slash)) posix = colon + 1;
      if (strncmp(posix, s_base_dir, s_base_len) != 0) break;
      const char *rest = posix + s_base_len;
      while (*rest == '/') rest++;
      if (strncmp(rest, s_base_dir + 1, s_base_len - 1) != 0) break;
      path = rest - 1;                     // the second copy, from its '/'
    }
  }

  if (path != orig) {
    static int n = 0;
    if (n < 64) { n++; debugPrintf("sanitize_path: \"%s\" -> \"%s\"\n", orig, path); }
  }
  return path;
}
...
int unlinkat_fake(int dirfd, const char *path, int flags) {
  ...
  return unlink(sanitize_path(path));
}
...
FILE *fopen_fake(const char *path, const char *mode) {
  ...
  if (!f) {
    static int fails = 0;
    if (fails < 200) { fails++; debugPrintf("fopen(%s, %s) -> FAIL\n", path, mode); }
    return NULL;
  }
  if (mode && strchr(mode, 'r')) {
    const char *ext = strrchr(path, '.');
    if (ext && (strcasecmp(ext, ".apk") == 0 || strcasecmp(ext, ".droid") == 0 ||
                strcasecmp(ext, ".dat") == 0 || strcasecmp(ext, ".bin") == 0))
      setvbuf(f, NULL, _IOFBF, 256 * 1024);
    // Log the first reads of the game's data. The runner zip_open()s game.apk
    // and inflates assets/game.droid out of it, so ".apk" should show up here
    // early in boot; a loose game.droid read means the APK path failed.
    if (path && (strstr(path, "game.droid") || strstr(path, ".apk") ||
                 strstr(path, ".ini") || strstr(path, ".sav"))) {
      static int n = 0;
...
int access_fake(const char *path, int mode) {
  return access(sanitize_path(path), mode);
}

int mkdir_fake(const char *path, mode_t mode) {
  return mkdir(sanitize_path(path), mode);
}

int rmdir_fake(const char *path) {
  return rmdir(sanitize_path(path));
}

void *opendir_fake(const char *path) {
  return opendir(sanitize_path(path));
}

int remove_fake(const char *path) {
  return remove(sanitize_path(path));
}

// POSIX rename() atomically replaces an existing destination; Horizon's FS
// refuses instead. The runner's zip writer and GML file_rename() rely on the
// POSIX behaviour, so emulate it (not atomic, but the only option).
int rename_fake(const char *oldpath, const char *newpath) {
  const char *o = sanitize_path(oldpath);
  const char *n = sanitize_path(newpath);
  if (rename(o, n) == 0) return 0;
  struct stat so, sn;
  if (stat(o, &so) == 0 && stat(n, &sn) == 0 && !S_ISDIR(sn.st_mode)) {
    unlink(n);
    return rename(o, n);
  }
  return -1;
...
int chdir_fake(const char *path) {
  return chdir(sanitize_path(path));
}
...
int stat_fake(const char *path, struct bionic_stat *st) {
  struct stat real;
  const int ret = stat(sanitize_path(path), &real);
...
void *readdir_fake(void *dirp) {
  static __thread struct bionic_dirent out; // per thread, so two directory walks cannot clobber each other
...
size_t fread_fake(void *ptr, size_t size, size_t n, FILE *f) { fsdev_guard_restore(); if (is_fake_file(f)) return 0; return fread(ptr, size, n, f); }
...
int fflush_fake(FILE *f) { fsdev_guard_restore(); if (is_fake_file(f) || f == NULL) return 0; return fflush(f); }
int fclose_fake(FILE *f) { fsdev_guard_restore(); if (is_fake_file(f)) return 0; return fclose(f); }
int ferror_fake(FILE *f) { if (is_fake_file(f)) return 0; return ferror(f); }
int fileno_fake(FILE *f) { fsdev_guard_restore(); if (is_fake_file(f)) return ((const uint8_t *)f - &fake_sF[0][0]) / 0x100; return fileno(f); }
...
int fseek_fake(FILE *f, long off, int whence) { fsdev_guard_restore(); if (is_fake_file(f)) return -1; return fseek(f, off, whence); }
int fseeko_fake(FILE *f, off_t off, int whence) { fsdev_guard_restore(); if (is_fake_file(f)) return -1; return fseeko(f, off, whence); }
...
// Statically-initialised bionic rwlocks are all-zero, so the real object is
// created on first use. Two threads can get here at once: serialise creation.
static Mutex s_rw_lazy;
static FakeRwLock *get_rwlock(void **storage) {
  FakeRwLock *l = *(FakeRwLock *volatile *)storage;
  if (l) return l;
  mutexLock(&s_rw_lazy);
  l = *storage;
  if (!l) {
...
// ---------------------------------------------------------------------------
// networking (the runner's HTTP layer): stubbed -- every call fails cleanly so
...
// ctype array
// BSD/bionic layout: _ctype_[0] is the EOF entry and lookups are
// (_ctype_ + 1)[c] -- libc++'s classic_table() returns _ctype_ + 1 on Android.
// fake_ctype[c + 128] holds the flags for c, so the pointer handed out must be
// &fake_ctype[127] (it was [128], shifting every class by one character).
// This runner does not import _ctype_, but keep the shim correct.
static char fake_ctype[384];
const char *ctype_fake_ptr = &fake_ctype[127];

void ctype_init(void) {
  for (int i = -128; i < 256; i++) {
    int c = i;
    char val = 0;
    if (c >= 'A' && c <= 'Z') val |= 0x01; // _U
    if (c >= 'a' && c <= 'z') val |= 0x02; // _L
    if (c >= '0' && c <= '9') val |= 0x04; // _N
    if (c == ' ' || c == '\t' || c == '\n' || c == '\r' || c == '\v' || c == '\f') val |= 0x08; // _S
    if ((c >= 33 && c <= 47) || (c >= 58 && c <= 64) || (c >= 91 && c <= 96) || (c >= 123 && c <= 126)) val |= 0x10; // _P
    if ((c >= 0 && c <= 31) || c == 127) val |= 0x20; // _C
    if ((c >= '0' && c <= '9') || (c >= 'a' && c <= 'f') || (c >= 'A' && c <= 'F')) val |= 0x40; // _X
    if (c == ' ' || c == '\t') val |= 0x80; // _B
    fake_ctype[c + 128] = val;
  }
}

size_t __strlcat_chk_fake(char *dst, const char *src, size_t siz, size_t dstlen) { (void)dstlen; return strlcat(dst, src, siz); }
size_t __strlcpy_chk_fake(char *dst, const char *src, size_t siz, size_t dstlen) { (void)dstlen; return strlcpy(dst, src, siz); }
int getuid_fake(void) { return 1000; }
const char *getprogname_fake(void) { return "pcl_nx"; }
void _exit_fake(int status) { exit(status); }
int fcntl_fake(int fd, int cmd, ...) {
  ...
  switch (cmd) {
    ...
    case 2: /* F_SETFD */ return 0;          // FD_CLOEXEC is meaningless here
    case 3: /* F_GETFL */ { const int r = fcntl(fd, 3, 0); return r < 0 ? 2 /* O_RDWR */ : r; }
    case 4: /* F_SETFL */ return 0;          // O_NONBLOCK on files: no-op
    default: return fcntl(fd, cmd, arg);
  }
}
int raise_fake(int sig) { (void)sig; return 0; }
int getpagesize_fake(void) { return 0x1000; }
int getppid_fake(void) { return 1; }
int getpgid_fake(int pid) { (void)pid; return 1; }
int getpriority_fake(int which, int who) { (void)which; (void)who; return 0; }
int sigpending_fake(void *set) { (void)set; return 0; }
int sigprocmask_fake(int how, const void *set, void *oset) { (void)how; (void)set; (void)oset; return 0; }
// bionic arm64 struct rusage: two timevals + 14 longs = 144 bytes.
int getrusage_fake(int who, void *usage) {
  (void)who;
  if (usage) {
    memset(usage, 0, 144);
    const u64 us = armTicksToNs(armGetSystemTick()) / 1000ull;   // uptime as "user time"
    int64_t *tv = (int64_t *)usage;
    tv[0] = (int64_t)(us / 1000000ull);
    tv[1] = (int64_t)(us % 1000000ull);
  }
  return 0;
}

// bionic arm64 struct statfs (asm-generic, 64-bit words): 120 bytes.
...
static void fill_statfs(void *buf) {
  struct bionic_statfs *s = buf;
  memset(s, 0, sizeof(*s));
  s->f_type = 0x4d44;                        // "MSDOS" (exFAT/FAT on the card)
  s->f_bsize = s->f_frsize = 0x1000;
  s->f_blocks = s->f_bfree = s->f_bavail = 256ull * 1024 * 1024;
  s->f_files = s->f_ffree = 1ull << 20;
  s->f_namelen = 255;
}
int statfs_fake(const char *path, void *buf) { (void)path; if (buf) fill_statfs(buf); return 0; }
int fstatfs_fake(int fd, void *buf) { (void)fd; if (buf) fill_statfs(buf); return 0; }
int fstatvfs_fake(int fd, void *buf) { (void)fd; return statvfs_fake(NULL, buf); }

// bionic arm64 struct sigaction is 32 bytes; report "default handler, no mask".
int sigaction_fake(int sig, const void *act, void *oact) {
  (void)sig; (void)act;
  if (oact) memset(oact, 0, 32);
  return 0;
}
int signal_fake(int sig, void *func) { (void)sig; (void)func; return 0; }
...
// bionic's struct mallinfo uses size_t fields (80 bytes on arm64); returning
// the 40-byte int version left half of the caller's struct as stack garbage.
struct fake_mallinfo {
  size_t arena, ordblks, smblks, hblks, hblkhd;
  size_t usmblks, fsmblks, uordblks, fordblks, keepcost;
};
struct fake_mallinfo mallinfo_fake(void) {
