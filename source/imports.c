/* imports.c -- .so import resolution for libyoyo.so
 *
...
// ---------------------------------------------------------------------------
// stack protector: the runner imports __stack_chk_guard as a data symbol. Its
// direct TPIDR_EL0 reads are redirected by so_patch_tpidr_stack_guard(), leaving
// the Switch runtime's thread-local state intact.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// pthread APIs return error numbers directly. Android's libc++ was compiled
// against Linux/bionic errno values, while devkitA64 newlib uses different
// numbers for a few POSIX errors (notably ETIMEDOUT: 110 vs 116). Returning a
// raw newlib timeout makes std::condition_variable throw std::system_error.
#define BIONIC_ETIMEDOUT 110
#define BIONIC_EDEADLK    35
#define BIONIC_ENOSYS     38

static int pthread_result_to_bionic(int rc) {
  if (rc == ETIMEDOUT) return BIONIC_ETIMEDOUT;
  if (rc == EDEADLK) return BIONIC_EDEADLK;
  if (rc == ENOSYS) return BIONIC_ENOSYS;
  return rc;
}
...
// Serialises lazy creation of statically-initialised bionic mutexes/condvars
// (see ensure_mutex). Values <= this are bionic static-initialiser constants.
static Mutex s_lazy_lock;
#define BIONIC_STATIC_MAX 0xFFFFu
...
static int pthread_mutex_destroy_fake(pthread_mutex_t **uid) {
  if (uid && *uid && (uintptr_t)*uid > BIONIC_STATIC_MAX) {
...
// bionic's static initialisers put a small constant in the first word:
//   PTHREAD_MUTEX_INITIALIZER             0
//   PTHREAD_RECURSIVE_MUTEX_INITIALIZER   0x4000  ((RECURSIVE & 3) << 14)
//   PTHREAD_ERRORCHECK_MUTEX_INITIALIZER  0x8000  ((ERRORCHECK & 3) << 14)
// The real newlib object is created on first use. Several threads can hit the
// same static mutex at once, so creation is serialised; otherwise two threads
// can each install their own mutex and both "own" the lock.
// (The old test was `< 0x8000`, which treated the errorcheck value as a pointer.)

static int ensure_mutex(pthread_mutex_t **uid) {
  if ((uintptr_t)*(pthread_mutex_t *volatile *)uid > BIONIC_STATIC_MAX) return 0;
  int ret = 0;
  mutexLock(&s_lazy_lock);
  const uintptr_t v = (uintptr_t)*uid;
  if (v <= BIONIC_STATIC_MAX) {
    int attr = (v == 0x4000) ? 1 : 0;       // recursive; errorcheck behaves as normal
    ret = pthread_mutex_init_fake(uid, &attr);
  }
  mutexUnlock(&s_lazy_lock);
  return ret;
}
...
static int ensure_cond(pthread_cond_t **cnd) {
  if (*(pthread_cond_t *volatile *)cnd) return 0;
  int ret = 0;
  mutexLock(&s_lazy_lock);
...
static int pthread_cond_timedwait_fake(pthread_cond_t **cnd, pthread_mutex_t **mtx, const struct timespec *t) {
  if (ensure_cond(cnd) < 0) return -1;
  if (ensure_mutex(mtx) < 0) return -1;
  const int rc = pthread_cond_timedwait(*cnd, *mtx, t);
  if (rc == ETIMEDOUT) {
    static unsigned timeout_logs = 0;
    const unsigned n = __atomic_fetch_add(&timeout_logs, 1, __ATOMIC_RELAXED);
    if (n < 4)
      debugPrintf("pthread_cond_timedwait: native ETIMEDOUT=%d -> bionic=%d deadline=%lld.%09ld\n",
                  rc, BIONIC_ETIMEDOUT, t ? (long long)t->tv_sec : -1LL,
                  t ? t->tv_nsec : 0L);
  }
  return pthread_result_to_bionic(rc);
}
...
// The bionic attr layout is incompatible, so it is ignored -- this runner never
// sets one anyway (it imports no pthread_attr_* setters). libnx's default stack
// is 128 KiB, Android's is ~1 MiB; the runner's job workers and audio thread
// were written for the latter, so ask for 1 MiB. The attr calls are weak so the
// build still links on a toolchain without them (then libnx's default is used).
#pragma weak pthread_attr_init
#pragma weak pthread_attr_destroy
#pragma weak pthread_attr_setstacksize
#define RUNNER_THREAD_STACK (1024 * 1024)

static int pthread_create_fake(pthread_t *thread, const void *unused, void *entry, void *arg) {
  (void)unused;
  if (pthread_attr_init && pthread_attr_destroy && pthread_attr_setstacksize) {
    pthread_attr_t attr;
    if (pthread_attr_init(&attr) == 0) {
      int rc = -1;
      if (pthread_attr_setstacksize(&attr, RUNNER_THREAD_STACK) == 0)
        rc = pthread_create(thread, &attr, (void *(*)(void *))entry, arg);
      pthread_attr_destroy(&attr);
      if (rc == 0) return 0;
    }
  }
  return pthread_create(thread, NULL, (void *(*)(void *))entry, arg);
}
...
// newlib=1), so the runner's clock_gettime(CLOCK_REALTIME) hits an unsupported
...
//
// The runner never draws straight into the panel: display.c gives it an
// offscreen target at the chosen game resolution and scales that texture onto
// the panel at the end of each frame.
//
// The runner normally learns its screen framebuffer by asking Java
// (GetDefaultFrameBuffer, answered in jni_fake.c), but any code path that binds
// 0 explicitly has to land on the same target, so catch that here.
//
// The offset path below is the fallback used when the offscreen target could
// not be created; display.c leaves render_offset_* at zero otherwise.
// ---------------------------------------------------------------------------

static GLuint s_bound_fbo = 0;

static void nx_glBindFramebuffer(GLenum target, GLuint fb) {
  if (fb == 0) {
    const GLuint redirect = display_game_fbo();
    if (redirect) fb = redirect;
  }
  if (target == GL_FRAMEBUFFER) s_bound_fbo = fb;
  glBindFramebuffer(target, fb);
}

// Only meaningful in the direct-to-panel fallback.
static void nx_glViewport(GLint x, GLint y, GLsizei w, GLsizei h) {
  if (s_bound_fbo == 0 && (render_offset_x | render_offset_y))
    glViewport(x + render_offset_x, y + render_offset_y, w, h);
  else
    glViewport(x, y, w, h);
}

static void nx_glScissor(GLint x, GLint y, GLsizei w, GLsizei h) {
  if (s_bound_fbo == 0 && (render_offset_x | render_offset_y))
    glScissor(x + render_offset_x, y + render_offset_y, w, h);
  else
    glScissor(x, y, w, h);
}

// main.c clears the full panel each frame; it needs the real binding state.
GLuint gl_bound_fbo(void) { return s_bound_fbo; }

static void *eglGetProcAddress_fake(const char *name) {
  void *p = (void *)eglGetProcAddress(name);
  debugPrintf("eglGetProcAddress('%s') -> %p\n", name ? name : "", p);
  return p;
}

// dlsym: the runner probes GL/EGL extensions through it. Resolve via eglGetProcAddress
...
// Functions we deliberately wrap. eglGetProcAddress would hand the runner
// Mesa's raw entry point and silently bypass the wrapper, so these resolve from
// our own table first.
static const char *const kOverrideNames[] = {
  "glBindFramebuffer",  // redirects the "screen" to the offscreen portrait target
  "glViewport",         // pillarbox offset in the direct-to-panel fallback
  "glScissor",
};

static int is_override(const char *name) {
  if (!name) return 0;
  for (unsigned i = 0; i < sizeof(kOverrideNames) / sizeof(*kOverrideNames); i++)
    if (!strcmp(name, kOverrideNames[i])) return 1;
  return 0;
}

// Canary for the GLES2-only import block. GLFuncImport() resolves its 131 GL
// pointers in three runs: 40 shared core, 48 GLES2-only (FBOs, shaders,
// glBlendEquationSeparate), then 43 GLES1/OES. Passing the wrong flag to
// initGLFuncs silently skips the middle block and the failure only surfaces
// later as a jump to address 0. Watch for a member of that block instead.
static int s_saw_gles2_import = 0;
int gl_saw_gles2_import(void) { return s_saw_gles2_import; }

static void *dlsym_fake(void *handle, const char *name) {
  (void)handle;
  debugPrintf("dlsym('%s')\n", name ? name : "");
  if (name && (!strcmp(name, "glBlendEquationSeparate") ||
               !strcmp(name, "glCreateProgram") ||
               !strcmp(name, "glGenFramebuffers")))
    s_saw_gles2_import = 1;
  if (is_override(name)) {
    DynLibFunction *o = so_find_import(dynlib_functions, (int)dynlib_numfunctions, name);
    if (o) {
      debugPrintf("dlsym('%s') -> wrapper override: %p\n", name, (void *)o->func);
      return (void *)o->func;
    }
  }
  void *p = (void *)eglGetProcAddress(name);
  if (p) {
    debugPrintf("dlsym('%s') -> found in eglGetProcAddress: %p\n", name, p);
    return p;
  }
  DynLibFunction *f = so_find_import(dynlib_functions, (int)dynlib_numfunctions, name);
  if (f) {
    debugPrintf("dlsym('%s') -> found in imports: %p\n", name, (void *)f->func);
    return (void *)f->func;
  }
  debugPrintf("dlsym('%s') -> NOT FOUND\n", name);
  return NULL;
}
// Some runner builds bundle Oboe, which probes AAudio first (this one uses
// YYOpenAL -> OpenSL ES directly, but the guard is harmless):
//   AAudioLoader::open() -> dlopen("libaaudio.so") -> dlsym(AAudioStreamBuilder_*)
// Returning a non-NULL handle for libaaudio.so makes Oboe believe AAudio exists
// and then fail on every symbol. Fail the dlopen instead and Oboe drops
// straight through to OpenSL ES, which opensles.c implements.
static void *dlopen_fake(const char *name, int flag) {
  (void)flag;
  if (name && strstr(name, "libaaudio")) {
    debugPrintf("dlopen('%s') -> NULL (forcing the OpenSL ES path)\n", name);
    return NULL;
  }
  return (void *)1;
}
...
// fall back to read()/fread() (the runner's file layer does).
// --- symbols POINPY needs that the HMD/STT import tables did not carry ------

// newlib has no clock_nanosleep. TIMER_ABSTIME (1) is the interesting case:
// convert the absolute deadline into a relative sleep.
#define NX_TIMER_ABSTIME 1
static int clock_nanosleep_fake(clockid_t clk, int flags,
                                const struct timespec *req, struct timespec *rem) {
  if (!req) return EINVAL;
  struct timespec delay = *req;
  if (flags & NX_TIMER_ABSTIME) {
    struct timespec now;
    if (clock_gettime(clk, &now) != 0) return EINVAL;
    delay.tv_sec  = req->tv_sec  - now.tv_sec;
    delay.tv_nsec = req->tv_nsec - now.tv_nsec;
    if (delay.tv_nsec < 0) { delay.tv_nsec += 1000000000L; delay.tv_sec--; }
    if (delay.tv_sec < 0) return 0; // deadline already passed
  }
  // nanosleep reports through errno; clock_nanosleep returns the error directly.
  if (nanosleep(&delay, rem) != 0) return errno;
  return 0;
}

// Scheduling policy is not something homebrew negotiates; SCHED_OTHER (0) is
// what bionic reports for an ordinary thread and is what the runner expects.
static int sched_getscheduler_fake(pid_t pid) { (void)pid; return 0; }
...
// an engine render target can request a 0-sized FBO texture
...
static void glTexImage2D_guard(GLenum target, GLint level, GLint internalformat,
  ...
  if (logged < 64) {
    logged++;
    debugPrintf("glTexImage2D(target=%u, level=%d, format=%d, width=%d, height=%d)\n",
                (unsigned)target, (int)level, (int)internalformat, (int)width, (int)height);
  }
  if (width <= 0 || height <= 0) {
    width = width <= 0 ? 1 : width;
    height = height <= 0 ? 1 : height;
    pixels = NULL; // the supplied buffer no longer matches the clamped size
    debugPrintf("  clamped degenerate texture to 1x1\n");
  }
  glTexImage2D(target, level, internalformat, width, height, border, format, type, pixels);
}

static void glShaderSource_fake(GLuint shader, GLsizei count, const GLchar *const *string, const GLint *length) {
  debugPrintf("glShaderSource(shader=%u, count=%d)\n", shader, count);
  if (count > 0 && string && string[0]) {
    char preview[101];
    strncpy(preview, string[0], 100);
    preview[100] = '\0';
    for (int i = 0; preview[i]; i++) {
      if (preview[i] == '\n' || preview[i] == '\r') preview[i] = ' ';
    }
    debugPrintf("  source preview: \"%s...\"\n", preview);
  }
  glShaderSource(shader, count, string, length);
}

static void glCompileShader_fake(GLuint shader) {
  debugPrintf("glCompileShader(shader=%u)\n", shader);
  glCompileShader(shader);
  GLint status = 0;
  glGetShaderiv(shader, GL_COMPILE_STATUS, &status);
  debugPrintf("  compile status: %d\n", status);
  if (status == GL_FALSE) {
    char log[1024];
    glGetShaderInfoLog(shader, sizeof(log), NULL, log);
    debugPrintf("  compile log: %s\n", log);
  }
}

static void glLinkProgram_fake(GLuint program) {
  debugPrintf("glLinkProgram(program=%u)\n", program);
  glLinkProgram(program);
  GLint status = 0;
  glGetProgramiv(program, GL_LINK_STATUS, &status);
  debugPrintf("  link status: %d\n", status);
  if (status == GL_FALSE) {
    char log[1024];
    glGetProgramInfoLog(program, sizeof(log), NULL, log);
    debugPrintf("  link log: %s\n", log);
  }
}

// Called many times per frame: never log here (with debug_log on, every line is
// a flushed SD write).
static void glUseProgram_fake(GLuint program) {
...
DynLibFunction dynlib_functions[] = {
  ...
  { "__assert2", (uintptr_t)&yy_assert2 },
  ...
  { "strcasestr", (uintptr_t)&yy_strcasestr },
  ...
  { "isalpha_l", (uintptr_t)&isalpha_l_fake },
  { "_ctype_", (uintptr_t)&ctype_fake_ptr },
  ...
  { "timegm", (uintptr_t)&yy_timegm },
  { "strftime", (uintptr_t)&strftime },
  { "time", (uintptr_t)&time },
  { "nanosleep", (uintptr_t)&nanosleep },
  { "clock_nanosleep", (uintptr_t)&clock_nanosleep_fake },
  { "sched_getscheduler", (uintptr_t)&sched_getscheduler_fake },
  ...
  // networking (the runner's HTTP layer) -- stubbed
  ...
  // EGL (only eglGetProcAddress is used by the runner; resolve to real EGL)
  ...
  { "glBindFramebuffer", (uintptr_t)&nx_glBindFramebuffer },
  ...
  { "glCompileShader", (uintptr_t)&glCompileShader_fake },
  ...
  { "glScissor", (uintptr_t)&nx_glScissor },
  ...
  // Oboe's EngineOpenSLES::open() requires this one too and fails the whole
  // stream if dlsym cannot find it. It was defined in opensles.c but never
  // exported here, which is why POINPY booted silent.
  ...
  // Socket, System and other new fakes
  ...
  { "if_indextoname", (uintptr_t)&yy_if_indextoname },
  { "getuid", (uintptr_t)&getuid_fake },
  { "getprogname", (uintptr_t)&getprogname_fake },
  ...
  { "getppid", (uintptr_t)&getppid_fake },
  { "getpgid", (uintptr_t)&getpgid_fake },
  { "getpriority", (uintptr_t)&getpriority_fake },
  { "sigpending", (uintptr_t)&sigpending_fake },
  { "sigprocmask", (uintptr_t)&sigprocmask_fake },
  { "getrusage", (uintptr_t)&getrusage_fake },
  { "statfs", (uintptr_t)&statfs_fake },
  { "statfs64", (uintptr_t)&statfs_fake },
  { "fstatfs", (uintptr_t)&fstatfs_fake },
  { "fstatvfs", (uintptr_t)&fstatvfs_fake },
  { "sigaction", (uintptr_t)&sigaction_fake },
  { "signal", (uintptr_t)&signal_fake },
  // The filter/compare callbacks expect bionic dirent64 records, so the plain
  // newlib scandir (which hands them newlib dirents) cannot be used.
  { "scandir", (uintptr_t)&yy_scandir64 },
  { "scandir64", (uintptr_t)&yy_scandir64 },
