...
int so_load(so_module *mod, const char *filename, void *base, size_t max_size) {
  ...
  // Keep a private RW page after the ELF image for Android's stack canary.
  // GameMaker normally reaches it through TPIDR_EL0; the Switch runtime owns
  // that register, so its MRS instructions are redirected to this page below.
  mod->stack_guard_offset = ALIGN_MEM(mod->load_size, 0x1000);
  if (mod->stack_guard_offset > max_size ||
      max_size - mod->stack_guard_offset < 0x1000) {
    res = -3;
    goto err_free_so;
  }
  mod->load_size = mod->stack_guard_offset + 0x1000;
...
int so_patch_tpidr_stack_guard(so_module *mod) {
  if (!mod || !mod->load_base || !mod->load_virtbase ||
      mod->stack_guard_offset + 0x30 > mod->load_size)
    return -1;

  const uintptr_t guard_load = (uintptr_t)mod->load_base + mod->stack_guard_offset;
  const uintptr_t guard_virt = (uintptr_t)mod->load_virtbase + mod->stack_guard_offset;
  *(uint64_t *)(guard_load + 0x28) = 0x0123456789ABCDEFull;
  ...
  for (int i = 0; i < mod->phnum; i++) {
    ...
    const size_t code_size = p->p_filesz & ~(size_t)3;
    for (size_t off = 0; off < code_size; off += sizeof(uint32_t)) {
      uint32_t *instruction = (uint32_t *)((uintptr_t)mod->load_base + p->p_vaddr + off);
      const uint32_t old = *instruction;

      // MRS Xt, TPIDR_EL0 (the low five bits select Xt).
      if ((old & 0xFFFFFFE0u) != 0xD53BD040u)
        continue;

      const uintptr_t pc = (uintptr_t)mod->load_virtbase + p->p_vaddr + off;
      const int64_t page_delta =
          ((int64_t)guard_virt - (int64_t)(pc & ~(uintptr_t)0xFFF)) >> 12;
      if (page_delta < -(1LL << 20) || page_delta >= (1LL << 20)) {
        debugPrintf("so_patch_tpidr_stack_guard: ADRP target out of range\n");
        return -2;
      }

      // ADRP Xt, guard_page. Existing LDR [Xt,#0x28] instructions then read
      // the shared immutable canary without changing libnx's TPIDR_EL0.
      const uint32_t rt = old & 0x1F;
      const uint64_t imm21 = (uint64_t)page_delta & 0x1FFFFF;
      *instruction = 0x90000000u |
                     (uint32_t)((imm21 & 3) << 29) |
  ...
  debugPrintf("so_patch_tpidr_stack_guard: redirected %d TPIDR_EL0 reads\n", patched);
  return patched;
}
...
uintptr_t so_try_find_addr_rx(so_module *mod, const char *symbol) {
  for (int i = 0; i < mod->num_syms; i++) {
    char *name = mod->dynstrtab + mod->syms[i].st_name;
    if (strcmp(name, symbol) == 0) {
      uintptr_t result = (uintptr_t)mod->load_virtbase + mod->syms[i].st_value;
      debugPrintf("so: found symbol %s at %p\n", symbol, (void *)result);
      return result;
    }
  }
  debugPrintf("so: symbol NOT found: %s\n", symbol);
  return 0;
}
