...
/* mouse passthrough (custom mode) */
static uint32_t s_mouse_btn = 0;
static int      s_mouse_wheel_frame = 0;

/* stick deadzone as a fraction of full deflection (drifting Joy-Cons) */
#define STICK_DEADZONE 0.12f
...
void nxp_init(const NxpConfig *cfg) {
  ...
  if (!s_cfg.custom_buttons)
    settings_load();                  /* restores stick/mouse/gyro sensitivities */
  if (s_cfg.custom_buttons) {
    if (s_cfg.cursor_mode == 1)      s_visible = 1;
    else if (s_cfg.cursor_mode == 2) s_visible = 0;
  }
...
static void do_touch(void) {
  ...
  for (int i = 0; i < count; i++) {
    ...
    float x, y;
    if (s_cfg.map_touch)      { s_cfg.map_touch(px, py, &x, &y); }
    else if (s_rotation == 1) { x =        py * (sw/ph); y = (pw-px) * (sh/pw); }
...
static int do_mouse(void) {
  ...
  if (wheel != 0 && s_cfg.mouse_passthrough) {
    s_mouse_wheel_frame += wheel;
  } else if (wheel != 0) {
  ...
  if (dx || dy) {
    ...
    s_cx += dgx; s_cy += dgy;
    clamp_cursor();
    if (s_visible <= 0 && !(s_cfg.custom_buttons && s_cfg.cursor_mode == 2))
...
static void nxp_update_custom(void);

void nxp_update(void) {
  if (!s_ready) return;
  padUpdate(&s_pad);
  if (s_cfg.custom_buttons) { nxp_update_custom(); return; }
...
/* ------------------------------------------------ host-owned buttons mode */

static void nxp_update_custom(void) {
  const int docked = is_docked();
  if (docked != s_was_docked) {
    if (s_cfg.cursor_mode == 0 && (s_visible < 0 || docked)) s_visible = docked ? 1 : 0;
    s_was_docked = docked;
  }
  if (s_visible < 0) s_visible = 0;

  s_nev = 0;
  s_mouse_wheel_frame = 0;

  if (!docked) do_touch();
  (void)do_mouse();                        /* movement; buttons/wheel kept for the host */
  if (!s_mouse_connected) s_mouse_btn = 0;

  const float cx0 = s_cx, cy0 = s_cy;
  do_gyro();

  HidAnalogStickState ls = padGetStickPos(&s_pad, 0);
  float fx = ls.x / 32767.0f, fy = ls.y / 32767.0f;
  const float mag = sqrtf(fx * fx + fy * fy);
  if (mag > STICK_DEADZONE) {
    /* rescale past the deadzone, then square for fine control near centre */
    float m = (mag - STICK_DEADZONE) / (1.0f - STICK_DEADZONE);
    if (m > 1.0f) m = 1.0f;
    const float k = (m * m) / mag;
    float dgx, dgy;
    nxp_rot_delta(nxp_ctrl_rot(), fx * k * s_stick_speed, -fy * k * s_stick_speed, &dgx, &dgy);
    s_cx += dgx; s_cy += dgy;
    clamp_cursor();
  }

  if ((s_cx != cx0 || s_cy != cy0) && s_visible <= 0 && s_cfg.cursor_mode != 2)
    s_visible = 1;                         /* any pointer motion brings it up */
}

void nxp_set_cursor_pos(float x, float y) {
  s_cx = x; s_cy = y;
  clamp_cursor();
}

void nxp_set_visible(int visible) {
  if (s_cfg.custom_buttons && s_cfg.cursor_mode == 2) { s_visible = 0; return; }
  if (s_cfg.custom_buttons && s_cfg.cursor_mode == 1) { s_visible = 1; return; }
  s_visible = visible ? 1 : 0;
}

void nxp_recenter(void) {
  s_cx = s_cfg.screen_w * 0.5f;
  s_cy = s_cfg.screen_h * 0.5f;
  nxp_set_visible(1);
  logf_("nxp: cursor recentered\n");
...
void nxp_toggle_gyro(void) {
  if (s_mouse_connected) { logf_("nxp: gyro stays OFF while a mouse is connected\n"); return; }
  if (!s_gyro_ready)     { logf_("nxp: gyro unavailable on this controller\n"); return; }
  s_gyro_on = !s_gyro_on;
  logf_("nxp: gyro %s\n", s_gyro_on ? "ON" : "OFF");
  if (s_gyro_on) nxp_set_visible(1);
}

void nxp_toggle_cursor(void) {
  nxp_set_visible(s_visible > 0 ? 0 : 1);
  logf_("nxp: cursor %s\n", s_visible > 0 ? "ON" : "OFF");
}

uint32_t nxp_mouse_buttons(void) { return s_mouse_btn; }
int      nxp_mouse_wheel(void)   { return s_mouse_wheel_frame; }
