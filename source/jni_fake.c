/* jni_fake.c -- fake JNI environment for the GameMaker Android runner (libyoyo.so)
 *
 * Adapted from POINPY NX for Pocket Crystal League: landscape window size,
 * no gamepads, clipboard / InputString / video / orientation answers added.
 *
 * This software may be modified and distributed under the terms
 * of the MIT license. See the LICENSE file for details.
 *
 * Implements a JNIEnv/JavaVM whose class lookups and method calls resolve to
 * native C, backing the RunnerActivity surface the runner calls back into:
 * preferences, device/locale queries, the gamepad bridge, the bitmap text
 * rasteriser, and extension calls via CallExtensionFunction (none in this game).
 * Preferences persist through prefs.c; text renders through gfx.c.
 */
...
#include "util.h"
#include "jni_fake.h"
void *g_dsMapCreate = NULL;
void *g_dsMapAddString = NULL;
void *g_dsMapAddInt = NULL;
void *g_onGPDeviceAdded = NULL;
void *g_onGamepadChange = NULL;
#include "asset.h"
#include "gfx.h"
#include "prefs.h"
...
enum {
  ...
  TAG_DOUBLE = 0x44424c31, // 'DBL1'  boxed java.lang.Double
};
...
typedef struct { uint32_t tag; double value; } FakeDouble;

volatile int jni_quit_requested = 0;

// ---------------------------------------------------------------------------
// log throttle: each distinct method/field/key name is logged a few times only.
// The runner polls some Java methods every frame, and with debug_log on every
// line is a flushed SD write.
// ---------------------------------------------------------------------------

#define LOG_SLOTS 1024
static uint32_t s_log_hash[LOG_SLOTS];
static uint8_t  s_log_count[LOG_SLOTS];

static int log_ok(const char *key) {
  if (!g_debug_log) return 0;
  uint32_t h = 2166136261u;
  for (const unsigned char *p = (const unsigned char *)(key ? key : ""); *p; p++) h = (h ^ *p) * 16777619u;
  if (!h) h = 1;
  for (unsigned i = 0, slot = h % LOG_SLOTS; i < 16; i++, slot = (slot + 1) % LOG_SLOTS) {
    if (s_log_hash[slot] == h) return s_log_count[slot] < 3 ? (s_log_count[slot]++, 1) : 0;
    if (!s_log_hash[slot]) { s_log_hash[slot] = h; s_log_count[slot] = 1; return 1; }
  }
  return 0;
}
#define JNI_LOG(key, ...) do { if (log_ok(key)) debugPrintf(__VA_ARGS__); } while (0)
int g_gamepad_added = 0;
...
static void *reg_local(void *ref) {
  if (ref) {
    ...
    else { static int n = 0; if (n < 8) { n++; debugPrintf("JNI: local ref table full, leaking %p\n", ref); } }
...
static void free_ref(void *ref) {
  ...
  switch (*(uint32_t *)ref) {
    ...
    case TAG_DOUBLE: free(ref); break;
...
// GameMaker extension functions return java.lang.Double (or String); the runner
// unboxes with doubleValue(). Box a real value so CallExtensionFunction can
// answer AndroidRumble*/GPBilling_*/GooglePlayServices_* calls.
void *jni_make_double(double v) {
  FakeDouble *d = calloc(1, sizeof(*d));
  d->tag = TAG_DOUBLE;
  d->value = v;
  return reg_local(d);
}
...
static FakeID *get_id(const char *cls, const char *name, const char *sig) {
  for (int i = 0; i < id_count; i++)
    if (!strcmp(id_pool[i].name, name) && !strcmp(id_pool[i].cls, cls ? cls : ""))
      return &id_pool[i];
  if (id_count >= MAX_IDS) { JNI_LOG(name, "JNI: id pool full (%s)\n", name); return &id_pool[0]; }
  debugPrintf("JNI: new id class='%s' name='%s' sig='%s'\n", cls ? cls : "", name ? name : "", sig ? sig : "");
...
// ---------------------------------------------------------------------------
// the runner's edit box (the runner's edit-box helper): track each field's text + max
...
// ---------------------------------------------------------------------------
// video helper (the runner's video helper): the engine's cutscenes are the obfuscated
...
static juint call_int(const char *name, va_list va) {
  // These are queried every frame. Return before logging: a flushed SD write
  // per JNI poll is dramatically more expensive than the call itself.
  // Frame pacing. Process() sets g_FrameCounting = (HasVsyncHandler() == 1) and
  // only then calls DoFrameTiming(), which sleeps to the next frame deadline
  // (Timing_Wait) and calls WaitForVsync. So:
  //   vsync 1 -> answer 0: runner does not sleep; eglSwapBuffers paces (Android default)
  //   vsync 0 -> answer 1: runner paces itself; swaps do not wait
  // Answering 0 with vsync off would let the game run unthrottled.
  if (!strcmp(name, "HasVsyncHandler")) return config.vsync ? 0 : 1;
  if (!strcmp(name, "isNetworkConnected")) return 1;
  if (!strcmp(name, "GetDefaultFrameBuffer")) return (juint)display_game_fbo();
  if (!strcmp(name, "GetOrientation")) return 0;   // natural landscape
  JNI_LOG(name, "JNI: call_int('%s')\n", name);
  ...
  if (!strcmp(name, "getIntegerForKey")) {
    ...
    JNI_LOG(k, "prefs: getInt(%s) = %d (def %d)\n", k, v, d);
    return (juint)v;
  }
  ...
  // The runner's "screen" is the offscreen game target, not the panel.
  if (!strcmp(name, "getGuiWidth") || !strcmp(name, "getScreenWidth")) return render_width;
  if (!strcmp(name, "getGuiHeight") || !strcmp(name, "getScreenHeight")) return render_height;
  if (!strcmp(name, "checkPermission") || !strcmp(name, "CheckPermission") || !strcmp(name, "checkCallingOrSelfPermission")) return 0;
  if (!strcmp(name, "UsingGL2")) return 1;
  if (!strcmp(name, "GLSupportsASTC")) {
    const char *ext = (const char *)glGetString(GL_EXTENSIONS);
    const int supported = ext &&
        (strstr(ext, "GL_KHR_texture_compression_astc_ldr") ||
         strstr(ext, "GL_OES_texture_compression_astc"));
    debugPrintf("JNI: GLSupportsASTC -> %d (KHR=%d OES=%d)\n",
                supported ? 1 : 0,
                ext && strstr(ext, "GL_KHR_texture_compression_astc_ldr") != NULL,
                ext && strstr(ext, "GL_OES_texture_compression_astc") != NULL);
    return supported ? 1 : 0;
  }

  if (!strcmp(name, "initGLFuncs")) return 1;
  if (!strcmp(name, "OsGetInfo")) {
    if (!g_dsMapCreate || !g_dsMapAddString || !g_dsMapAddInt) {
      debugPrintf("JNI: OsGetInfo FAILED, GMS2 map functions not resolved!\n");
      return 0;
    }
    
    int (*dsMapCreate)(void *, void *) = (void *)g_dsMapCreate;
    void (*dsMapAddString)(void *, void *, int, void *, void *) = (void *)g_dsMapAddString;
    void (*dsMapAddInt)(void *, void *, int, void *, int) = (void *)g_dsMapAddInt;
    
    int map_id = dsMapCreate(fake_env, NULL);
    debugPrintf("JNI: OsGetInfo created map_id=%d\n", map_id);
    
    dsMapAddString(fake_env, NULL, map_id, jni_make_string("os"), jni_make_string("android"));
    dsMapAddString(fake_env, NULL, map_id, jni_make_string("version"), jni_make_string("10"));
    dsMapAddInt(fake_env, NULL, map_id, jni_make_string("sdk_int"), 29);
    dsMapAddString(fake_env, NULL, map_id, jni_make_string("device"), jni_make_string("switch"));
    dsMapAddString(fake_env, NULL, map_id, jni_make_string("model"), jni_make_string("nintendo_switch"));
    dsMapAddString(fake_env, NULL, map_id, jni_make_string("manufacturer"), jni_make_string("nintendo"));
    
    return (juint)map_id;
  }
  // AudioTrack.getMinBufferSize(sampleRate, channelConfig, audioFormat) -> bytes
  if (!strcmp(name, "getMinBufferSize")) return 4096;
  // AudioTrack.write(byte[], offset, size) -> bytes written
  if (!strcmp(name, "write")) {
    int size = 0;
    (void)va_arg(va, void *); // byte[]
    (void)va_arg(va, int);    // offset
    size = va_arg(va, int);   // size
    return (juint)size; // pretend we consumed everything
  }
  // GMS2 misc int returns
  if (!strcmp(name, "GamepadsCount")) return 0; // mouse/keyboard game; no pads
  if (!strcmp(name, "GamepadGMLMapping")) return 0;
  if (!strcmp(name, "ShowQuestion")) return 0; // always "No"
  if (!strcmp(name, "VirtualKeyboardGetHeight")) return 0;
  if (!strcmp(name, "PushGetLocalNotification")) return 0;
  if (!strcmp(name, "DynamicAssetExists")) return 0;
  // Volume queries must not default to 0 -- classes.dex calls
  // AudioManager.getStreamVolume/getStreamMaxVolume, and answering 0 for the
  // current level would mute the game just as thoroughly as a dead stream.
  if (strstr(name, "getStreamMaxVolume")) return 15;
  if (strstr(name, "getStreamVolume"))    return 15;

  (void)va;
  JNI_LOG(name, "JNI: call_int('%s') UNHANDLED\n", name);
  return 0;
}

static juint call_bool(const char *name, va_list va) {
  if (!strcmp(name, "isNetworkConnected")) return 1; // hot per-frame poll
  JNI_LOG(name, "JNI: call_bool('%s')\n", name);
  if (!strcmp(name, "getBoolForKey")) {
    const char *k = obj_str(va_arg(va, void *));
    int d = va_arg(va, int);
    int v = prefs_get_bool(k, d);
    JNI_LOG(k, "prefs: getBool(%s) = %d (def %d)\n", k, v, d);
    return (juint)v;
  }
  ...
  if (!strcmp(name, "GamepadConnected")) return 0;
  if (!strcmp(name, "clipboardHasText")) return 0;
  if (!strcmp(name, "restrictOrientation") || !strcmp(name, "setupOrientation")) return 1;
  // PackageManager.hasSystemFeature: Switch has no TV/leanback/etc.
  if (!strcmp(name, "hasSystemFeature")) {
    (void)va_arg(va, void *); // consume the feature name string
    return 0;
  }
  // AudioTrack.play() returns bool in some contexts -- just return 1
  if (!strcmp(name, "PlayingMP3")) return 0; // not playing MP3
  if (!strcmp(name, "VirtualKeyboardGetStatus")) return 0;
  (void)va;
  JNI_LOG(name, "JNI: call_bool('%s') UNHANDLED\n", name);
  return 0;
}

static float call_float(const char *name, va_list va) {
  JNI_LOG(name, "JNI: call_float('%s')\n", name);
  ...
  (void)va;
  JNI_LOG(name, "JNI: call_float('%s') UNHANDLED\n", name);
  return 0.0f;
}

static double call_double(const char *name, va_list va) {
  // The runner polls this several times per frame even when no movie exists.
  if (!strncmp(name, "Video", 5))
    return 0.0;   // no video in this game: GetStatus/Duration/Position/W/H...
  JNI_LOG(name, "JNI: call_double('%s')\n", name);
  ...
  (void)va;
  JNI_LOG(name, "JNI: call_double('%s') UNHANDLED\n", name);
  return 0.0;
}

static juint call_long(const char *name, va_list va) {
  JNI_LOG(name, "JNI: call_long('%s')\n", name);
  (void)va;
  JNI_LOG(name, "JNI: call_long('%s') UNHANDLED\n", name);
  return 0;
}

static void *call_object(const char *name, va_list va) {
  JNI_LOG(name, "JNI: call_object('%s')\n", name);
  ...
  if (!strcmp(name, "getCocos2dxPackageName") || !strcmp(name, "getPackageName"))
    return jni_make_string(GAME_PACKAGE);
  if (!strcmp(name, "getDeviceModel")) return jni_make_string("Nintendo Switch");
  if (!strcmp(name, "getVersion")) return jni_make_string(GAME_VERSION);
  ...
  if (!strcmp(name, "getAssets") || !strcmp(name, "GetAssetManager")) return jni_make_object("AssetManager");
  if (!strcmp(name, "getFilesDir") || !strcmp(name, "getCacheDir") || !strcmp(name, "getExternalFilesDir")) return jni_make_object("File");
  if (!strcmp(name, "getAbsolutePath")) return jni_make_string(writable_dir());
  if (!strcmp(name, "GamepadDescription")) {
    int device_id = va_arg(va, int);
    if (device_id == 0) {
      return jni_make_string("Nintendo Switch Controller");
    }
    return NULL;
  }
  if (!strcmp(name, "GetUDID")) return jni_make_string("1234567890");
  if (!strcmp(name, "GetAppID")) return jni_make_string(GAME_PACKAGE);
  ...
  if (!strcmp(name, "getSystemService")) return jni_make_object("SystemService");
  if (!strcmp(name, "getApplicationContext") || !strcmp(name, "GetApplicationContext")) return jni_make_object("Context");
  // GMS2 extension / dynamic asset / certificate calls -- return safe stubs
  if (!strcmp(name, "LoadDynamicAsset")) return NULL; // no dynamic assets on Switch
  if (!strcmp(name, "CallExtensionFunction")) {
    // RunnerActivity.CallExtensionFunction(String cls, String func, int argc,
    //                                      double[] args, Object[] objArgs)
    const char *ext_cls  = obj_str(va_arg(va, void *));
    const char *ext_func = obj_str(va_arg(va, void *));
    const int   argc     = va_arg(va, int);
    FakePriArray *dargs  = va_arg(va, void *);
    FakeObjArray *oargs  = va_arg(va, void *);
    const double *dv = (dargs && dargs->tag == TAG_PRIARR) ? (const double *)dargs->data : NULL;
    const int dv_len = (dargs && dargs->tag == TAG_PRIARR) ? dargs->len : 0;
    const char *sv = NULL;
    if (oargs && oargs->tag == TAG_OBJARR && oargs->len > 0 && oargs->items)
      sv = obj_str(oargs->items[0]);
    return ext_call(ext_cls, ext_func, argc, dv, dv_len, sv);
  }
  if (!strcmp(name, "EnumerateCertificates")) return NULL; // no SSL certs needed
  if (!strcmp(name, "clipboardGetText")) return jni_make_string("");
  if (!strcmp(name, "InputString")) {
    // get_string(): blocking text entry -> Switch software keyboard
    const char *caption = obj_str(va_arg(va, void *));
    const char *def = obj_str(va_arg(va, void *));
    char out[256];
    snprintf(out, sizeof(out), "%s", def ? def : "");
    SwkbdConfig kbd;
    if (R_SUCCEEDED(swkbdCreate(&kbd, 0))) {
      swkbdConfigMakePresetDefault(&kbd);
      if (caption && caption[0]) swkbdConfigSetHeaderText(&kbd, caption);
      if (def && def[0]) swkbdConfigSetInitialText(&kbd, def);
      char tmp[256] = {0};
      if (R_SUCCEEDED(swkbdShow(&kbd, tmp, sizeof(tmp))))
        snprintf(out, sizeof(out), "%s", tmp);
      swkbdClose(&kbd);
    }
    return jni_make_string(out);
  }
  if (!strcmp(name, "cloudStringSave") || !strcmp(name, "cloudSynchronise")) return NULL;
  (void)va;
  JNI_LOG(name, "JNI: call_object('%s') UNHANDLED\n", name);
  return NULL;
}

static void call_void(const char *name, va_list va) {
  if (!strcmp(name, "WaitForVsync")) return;   // may be polled every frame
  JNI_LOG(name, "JNI: call_void('%s')\n", name);
  if (!strncmp(name, "Video", 5)) return;      // VideoOpen/Close/Pause/Resume/Seek/...
  if (!strcmp(name, "clipboardSetText") || !strcmp(name, "OpenURL") ||
      !strcmp(name, "AcquireMulticastLock") || !strcmp(name, "ReleaseMulticastLock"))
    return;
  // UserDefault writers
  ...
  // the runner's edit box: track text + length + the open request; the geometry/
  ...
  if (!strcmp(name, "terminateProcess") || !strcmp(name, "finish") || !strcmp(name, "exitGame") || !strcmp(name, "ExitApplication")) {
  ...
  if (!strcmp(name, "restrictOrientation") || !strcmp(name, "setupOrientation") || !strcmp(name, "RestrictOrientation")) return;
  if (!strcmp(name, "hideSplash") || !strcmp(name, "showSplash")) return;
  if (!strcmp(name, "powersaveEnable")) return;
  // GMS2 runner gamepad/network/analytics/cloud no-ops
  if (!strcmp(name, "ClearGamepads")) return;
  if (!strcmp(name, "EnumerateGamepadDevices")) {
    // Deliberately reports no controller. Pocket Crystal League never calls a
    // gamepad_* function; all physical input is translated to mouse, touch and
    // keyboard events by input.c.
    return;
  }
  if (!strcmp(name, "analyticsEvent") || !strcmp(name, "analyticsEventExt")) return;
  if (!strcmp(name, "cloudStringSave") || !strcmp(name, "cloudSynchronise")) return;
  if (!strcmp(name, "RequestPermission") || !strcmp(name, "MoveTaskToBack")) return;
  if (!strcmp(name, "DumpUsedMemory")) return;
  if (!strcmp(name, "PushLocalNotification") || !strcmp(name, "PushCancelLocalNotification")) return;
  if (!strcmp(name, "WaitForVsync")) return;
  if (!strcmp(name, "VirtualKeyboardToggle") || !strcmp(name, "OnKeyboardStringSet")) return;
  if (!strcmp(name, "setSystemUIVisibilityFlags")) return;
  if (!strcmp(name, "ShowMessage") || !strcmp(name, "ShowMessageAsync") ||
      !strcmp(name, "InputStringAsync") || !strcmp(name, "ShowQuestionAsync") ||
      !strcmp(name, "ShowLogin")) return;
  // AudioTrack lifecycle -- actual audio goes through OpenSLES in opensles.c
  if (!strcmp(name, "play") || !strcmp(name, "stop") || !strcmp(name, "release")) return;
  // MP3 player (handled by opensles or ignored)
  if (!strcmp(name, "PlayMP3") || !strcmp(name, "StopMP3") || !strcmp(name, "PauseMP3") ||
      !strcmp(name, "ResumeMP3") || !strcmp(name, "SetMP3Volume")) return;
  // Network / ratings / HTTP -- no-op on Switch
  if (!strcmp(name, "LeaveRating") || !strcmp(name, "HttpGet") ||
      !strcmp(name, "HttpPost") || !strcmp(name, "HttpRequest")) return;
  (void)va;
  JNI_LOG(name, "JNI: call_void('%s') UNHANDLED\n", name);
}

static void *j_GetObjectField(void *env, void *obj, FakeID *fid) {
  (void)env; (void)obj;
  JNI_LOG(fid->name, "JNI: GetObjectField('%s')\n", fid->name);
  if (!strcmp(fid->name, "versionName")) {
    return jni_make_string(GAME_VERSION);
  }
  JNI_LOG(fid->name, "JNI: GetObjectField('%s') UNHANDLED\n", fid->name);
  return NULL;
}

static juint j_GetIntField(void *env, void *obj, FakeID *fid) {
  (void)env; (void)obj;
  JNI_LOG(fid->name, "JNI: GetIntField('%s')\n", fid->name);
  if (!strcmp(fid->name, "versionCode")) {
    return GAME_VERSION_CODE;
  }
  JNI_LOG(fid->name, "JNI: GetIntField('%s') UNHANDLED\n", fid->name);
  return 0;
}

static void *j_GetStaticObjectField(void *env, void *cls, FakeID *fid) {
  (void)env; (void)cls;
  JNI_LOG(fid->name, "JNI: GetStaticObjectField('%s')\n", fid->name);
  if (!strcmp(fid->name, "MODEL")) {
    return jni_make_string("Nintendo Switch");
  }
  if (!strcmp(fid->name, "MANUFACTURER")) {
    return jni_make_string("Nintendo");
  }
  if (!strcmp(fid->name, "PRODUCT")) {
    return jni_make_string("Nintendo Switch");
  }
  // RunnerJNILib.ms_context: return a fake Context object
  if (!strcmp(fid->name, "ms_context")) {
    return jni_make_object("Context");
  }
  JNI_LOG(fid->name, "JNI: GetStaticObjectField('%s') UNHANDLED\n", fid->name);
  return NULL;
}

static juint j_GetStaticIntField(void *env, void *cls, FakeID *fid) {
  (void)env; (void)cls;
  JNI_LOG(fid->name, "JNI: GetStaticIntField('%s')\n", fid->name);
  if (!strcmp(fid->name, "SDK_INT")) {
    return 28;
  }
  if (strstr(fid->name, "RefreshRate")) return 60;
  JNI_LOG(fid->name, "JNI: GetStaticIntField('%s') UNHANDLED\n", fid->name);
  return 0;
}
...
static void *j_FindClass(void *env, const char *name) {
  (void)env;
  JNI_LOG(name, "JNI: FindClass('%s')\n", name ? name : "");
  return get_class(name);
}
static void *j_GetObjectClass(void *env, void *obj) {
  (void)env;
  if (obj) {
    const uint32_t tag = *(uint32_t *)obj;
    if (tag == TAG_STRING) return get_class("java/lang/String");
    if (tag == TAG_DOUBLE) return get_class("java/lang/Double");
  }
  return get_class("java/lang/Object");
}

// The GMS2 runner decides how to read an extension's return value by asking
// IsInstanceOf(result, String). Answering "yes" unconditionally would send a
// boxed Double through GetStringUTFChars, so be honest about the two types we
// actually box and stay permissive for everything else.
static juint j_IsInstanceOf(void *env, void *obj, void *cls) {
  (void)env;
  FakeObject *c = cls;
  if (!obj || !c || c->tag != TAG_CLASS) return 1;
  const uint32_t tag = *(uint32_t *)obj;
  if (strstr(c->label, "String")) return tag == TAG_STRING;
  if (strstr(c->label, "Double") || strstr(c->label, "Float") || strstr(c->label, "Number"))
...
// Number.doubleValue()/floatValue()/intValue() on a box we created ourselves.
// Anything else falls through to the regular by-name dispatch.
static double unbox_or(void *recv, const char *name, double fallback) {
  FakeDouble *d = recv;
  if (d && d->tag == TAG_DOUBLE &&
      (!strcmp(name, "doubleValue") || !strcmp(name, "floatValue") ||
       !strcmp(name, "intValue")    || !strcmp(name, "longValue")))
    return d->value;
  return fallback;
}
...
CALL_INST(j_CallIntMethod, juint, (juint)unbox_or(recv, id->name, (double)call_int(id->name, va)))
CALL_INST(j_CallBooleanMethod, juint, call_bool(id->name, va))
CALL_INST(j_CallLongMethod, juint, call_long(id->name, va))
CALL_INST(j_CallFloatMethod, float, (float)unbox_or(recv, id->name, (double)call_float(id->name, va)))
CALL_INST(j_CallDoubleMethod, double, unbox_or(recv, id->name, call_double(id->name, va)))
...
// --- CallStatic<type>Method (the runner uses these heavily) ----------------
...
// --- strings ----------------------------------------------------------------
// the runner's jstring accessor, the hot path for every jstring the
...
static juint j_unimplemented(void) {
  static int n = 0;
  if (n < 16) { n++; debugPrintf("JNI: unimplemented slot called\n"); }
  return 0;
}
