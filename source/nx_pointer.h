...
typedef struct {
  ...
  /* HOST-OWNED BUTTONS (added for pcl_nx).
   * When custom_buttons is nonzero the module reads NO pad buttons at all: no
   * +/- toggles, no L/R recenter, no D-pad sensitivity, and it never emits
   * cursor tap events. It only moves the cursor (left stick, gyro when enabled,
   * USB mouse) and reports touches. The host reads nxp_cursor_pos() and does its
   * own clicking, and drives the toggles through the nxp_* calls below. The
   * sensitivity file (pointer.cfg) is not used in this mode.                  */
  int   custom_buttons;
  /* custom mode only: 0 = auto (cursor appears on stick/gyro/mouse motion),
   * 1 = always visible, 2 = never drawn (pointer still moves).                */
  int   cursor_mode;
  /* USB mouse wheel and buttons are reported to the host (nxp_mouse_wheel,
   * nxp_mouse_buttons) instead of tuning sensitivity / tapping.               */
  int   mouse_passthrough;
  /* Optional: map a touch-panel point (panel_w x panel_h space) to render
   * space. Needed when the image does not fill the panel (letterboxing).     */
  void (*map_touch)(float px, float py, float *x, float *y);
} NxpConfig;
...
/* Host control (custom_buttons mode, but safe in any mode). */
void  nxp_set_cursor_pos(float x, float y);
void  nxp_set_visible(int visible);   /* ignored when cursor_mode == never   */
void  nxp_recenter(void);
void  nxp_toggle_gyro(void);
void  nxp_toggle_cursor(void);
uint32_t nxp_mouse_buttons(void);     /* HidMouseButton_* bits, this frame   */
int   nxp_mouse_wheel(void);          /* wheel notches this frame, + = up    */
