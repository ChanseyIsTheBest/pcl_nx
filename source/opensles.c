 ...
 * Players are software-mixed into one audout device; the buffer-queue completion
 * callback fires from the audio worker, like Android's fast-track callback.
 */
...
//
// Android adds SL_ANDROID_DATAFORMAT_PCM_EX, which is the same layout plus a
// trailing `representation` field saying whether the samples are ints or
// floats. Oboe uses it whenever it negotiates a float stream -- which it does
// here now that we report API 28 -- so recognising only plain PCM meant falling
// back to defaults and then decoding float32 buffers as int16.
...
typedef struct {
  ...
  SLuint32 endianness;
  SLuint32 representation;   // PCM_EX only
} SLDataFormat_PCM;
...
// SLAndroidConfigurationItf -- Android's OpenSL ES extension. Oboe requires it:
// EngineOpenSLES::open() resolves slCreateEngine, SL_IID_ENGINE,
// SL_IID_ANDROIDSIMPLEBUFFERQUEUE and SL_IID_ANDROIDCONFIGURATION up front and
// aborts the whole stream with ErrorInternal if any of the four is missing --
// before it ever calls slCreateEngine. Method order matches
// SLES/OpenSLES_AndroidConfiguration.h.
typedef struct SLAndroidConfigurationItf_ {
  SLresult (*SetConfiguration)(void *self, const char *key, const void *value, SLuint32 size);
  SLresult (*GetConfiguration)(void *self, const char *key, SLuint32 *size, void *value);
...
typedef struct Player {
  ...
  const SLAndroidConfigurationItf_ *cfg_vt;

  int in_use;
  int channels;
  int rate;
  int is_float;              // samples are 32-bit float, not 16-bit int
  int frame_bytes;           // channels * bytes per sample
  int started;               // logged the first buffer yet?
  SLuint32 cfg_perf_mode;    // SL_ANDROID_PERFORMANCE_NONE
  SLuint32 cfg_stream_type;
  // Switch audout runs at a fixed 48 kHz. The runner asks for 44.1 kHz, so step
  // through the source at rate/dev_rate frames per output frame (16.16 fixed).
  uint32_t step;
  uint32_t frac;
  int playing;
...
// --- FMV audio ring (fed by movie_player.c, drained by the audout callback) ---
...
// Decode one frame to a pair of 16-bit-scaled samples, whatever the source
// format is. Oboe hands us float32 when it negotiates a float stream and int16
// otherwise; GameMaker's own sounds arrive through the same queue.
static inline void read_frame(const Player *p, const uint8_t *base,
                              int32_t *out_l, int32_t *out_r) {
  if (p->is_float) {
    const float *f = (const float *)(const void *)base;
    float fl = f[0];
    float fr = (p->channels >= 2) ? f[1] : f[0];
    if (fl >  1.0f) fl =  1.0f;
    if (fl < -1.0f) fl = -1.0f;
    if (fr >  1.0f) fr =  1.0f;
    if (fr < -1.0f) fr = -1.0f;
    *out_l = (int32_t)(fl * 32767.0f);
    *out_r = (int32_t)(fr * 32767.0f);
  } else {
    const int16_t *sm = (const int16_t *)(const void *)base;
    *out_l = sm[0];
    *out_r = (p->channels >= 2) ? sm[1] : sm[0];
  }
}
...
static void mix_player(Player *p, int32_t *acc, int frames) {
  ...
  for (int i = 0; i < frames; i++) {
    if (!p->cur || p->cur_pos >= p->cur_size) {
      ...
      if (p->cur) {
        ...
        if (p->cb) {
          ...
          // Verify if player was destroyed/freed during the callback
          int still_valid = 0;
      ...
      if (p->lock) SDL_LockMutex(p->lock);
      ...
      if (p->lock) SDL_UnlockMutex(p->lock);
      if (!have) {
        // Underrun. A few at startup are normal; a steady stream of them means
        // the engine is not refilling, which is a different bug from silence.
        static int under = 0;
        if ((under++ & 0x3F) == 0)
          debugPrintf("opensles: buffer queue underrun (#%d)\n", under);
        break; // rest of this block is silence
      }
      p->cur = b.data;
      p->cur_size = b.size;
      p->cur_pos = 0;
    ...
    const SLuint32 frame_bytes = (SLuint32)(p->frame_bytes > 0 ? p->frame_bytes : 4);
    int32_t l, r;
    read_frame(p, p->cur + p->cur_pos, &l, &r);

    // Linear interpolation towards the next frame, but only when it sits inside
    // the same enqueued buffer -- straddling a boundary would read past the end,
    // and one nearest-neighbour frame every few milliseconds is inaudible.
    if (p->step != (1u << 16) && p->frac &&
        p->cur_pos + frame_bytes * 2 <= p->cur_size) {
      int32_t nl, nr;
      read_frame(p, p->cur + p->cur_pos + frame_bytes, &nl, &nr);
      const int32_t t = (int32_t)(p->frac >> 8);   // 0..255
      l += ((nl - l) * t) >> 8;
      r += ((nr - r) * t) >> 8;
    }
    ...
    // Advance the source by step/65536 frames.
    p->frac += p->step;
    p->cur_pos += (p->frac >> 16) * frame_bytes;
    p->frac &= 0xFFFFu;
  }
}
...
static void audio_callback(void *ud, int16_t *stream, int frames) {
  (void)ud;
  static int32_t acc[8192 * 2];
  if (frames > 8192) { memset(stream, 0, frames * 4); return; }
...
static void ensure_device(int rate) {
  ...
  if (nx_audio_is_ready())
    return;
  if (!nx_audio_init(rate, audio_callback, NULL)) {
    debugPrintf("opensles: nx_audio_init failed\n");
    return;
  }
  g_dev_rate = nx_audio_get_rate();
  debugPrintf("opensles: audout ready requested=%d device=%d\n", rate, g_dev_rate);
}
...
static SLresult bq_Enqueue(void *self, const void *pBuffer, SLuint32 size) {
  ...
  p->q_tail = next;
  const int first = !p->started;
  p->started = 1;
  if (p->lock) SDL_UnlockMutex(p->lock);
  if (first)
    debugPrintf("opensles: first Enqueue %u bytes (%u frames)\n",
                (unsigned)size, (unsigned)(p->frame_bytes ? size / (SLuint32)p->frame_bytes : 0));
...
static SLresult play_SetPlayState(void *self, SLuint32 state) {
  ...
  const int now = p->playing;
  if (p->lock) SDL_UnlockMutex(p->lock);
  if (was != now)
    debugPrintf("opensles: SetPlayState -> %s\n", now ? "PLAYING" : "STOPPED/PAUSED");
  return SL_RESULT_SUCCESS;
}
...
// Performance mode, stream type and content type are all Android mixer policy
// with no meaning here: we own the audout device outright. Accept and ignore.
// Reporting success keeps Oboe on its normal path -- a failure here makes it
// downgrade its own performance-mode bookkeeping for no benefit.
// Oboe sets these before Realize and then reads the performance mode back in
// updateStreamParameters(). Android's own GetConfiguration was buggy before
// API 28, so Oboe forces the result to SUCCESS when getSdkVersion() <= 27 --
// but we now report 28, so a FEATURE_UNSUPPORTED here propagates straight to
// "goto error" and fails the whole stream. Store what we are told and hand the
// same value back.
static SLresult cfg_SetConfiguration(void *self, const char *key,
                                     const void *value, SLuint32 size) {
  Player *p = CONTAINER(self, Player, cfg_vt);
  if (key && value && size >= sizeof(SLuint32)) {
    const SLuint32 v = *(const SLuint32 *)value;
    if (!strcmp(key, "androidPerformanceMode"))          p->cfg_perf_mode = v;
    else if (!strcmp(key, "androidPlaybackStreamType"))  p->cfg_stream_type = v;
    debugPrintf("opensles: SetConfiguration('%s') = %u\n", key, (unsigned)v);
  }
  return SL_RESULT_SUCCESS;
}
...
static SLresult cfg_GetConfiguration(void *self, const char *key,
  ...
  SLuint32 out;
  if (!strcmp(key, "androidPerformanceMode"))          out = p->cfg_perf_mode;
  else if (!strcmp(key, "androidPlaybackStreamType"))  out = p->cfg_stream_type;
  else return SL_RESULT_FEATURE_UNSUPPORTED;

  if (!value) { *size = sizeof(SLuint32); return SL_RESULT_SUCCESS; }
  if (*size < sizeof(SLuint32)) return SL_RESULT_PARAMETER_INVALID;
  *(SLuint32 *)value = out;
  *size = sizeof(SLuint32);
  debugPrintf("opensles: GetConfiguration('%s') -> %u\n", key, (unsigned)out);
  return SL_RESULT_SUCCESS;
}

// Routing through a Java AudioTrack/AudioRecord proxy. There is no Java here.
static SLresult cfg_AcquireJavaProxy(void *self, SLuint32 type, void **proxy) {
  (void)self; (void)type;
  if (proxy) *proxy = NULL;
  return SL_RESULT_FEATURE_UNSUPPORTED;
}
static SLresult cfg_ReleaseJavaProxy(void *self, SLuint32 type) {
  (void)self; (void)type;
  return SL_RESULT_FEATURE_UNSUPPORTED;
}

static const SLAndroidConfigurationItf_ player_cfg_vtable = {
...
static SLresult eng_CreateAudioPlayer(void *self, SLObjectItf *pPlayer, SLDataSource *src, SLDataSink *snk,
  ...
  debugPrintf("OpenSLES: eng_CreateAudioPlayer called\n");
  if (!pPlayer)
    return SL_RESULT_PARAMETER_INVALID;
  ...
  if (src && src->pFormat) {
    ...
    if (fmt->formatType == SL_DATAFORMAT_PCM ||
      ...
      if (fmt->formatType == SL_ANDROID_DATAFORMAT_PCM_EX &&
        ...
        p->is_float = 1;   // 32-bit without a representation field is float
    } else {
      debugPrintf("OpenSLES: UNKNOWN format type 0x%x -- assuming 16-bit PCM\n",
                  (unsigned)fmt->formatType);
    }
  }
  p->frame_bytes = p->channels * (p->is_float ? 4 : 2);
  debugPrintf("OpenSLES: player format rate=%d channels=%d sample=%s frame=%d bytes\n",
              p->rate, p->channels, p->is_float ? "float32" : "int16", p->frame_bytes);

  ensure_device(p->rate);

  // Fixed 16.16 resample ratio from the player rate to the device rate. This
  // MUST be set: calloc leaves it at 0, and a zero step means cur_pos never
  // advances -- the first frame repeats forever, the buffer never completes,
  // the completion callback never fires and the whole queue stalls silently.
  p->step = (uint32_t)(((uint64_t)p->rate << 16) /
                       (uint32_t)(g_dev_rate > 0 ? g_dev_rate : 48000));
  if (p->step == 0) p->step = (1u << 16);
  p->frac = 0;
  if (p->rate != g_dev_rate)
    debugPrintf("opensles: resampling %d -> %d Hz (step=%u/65536)\n",
                p->rate, g_dev_rate, (unsigned)p->step);
...
static SLresult eng_CreateOutputMix(void *self, SLObjectItf *pMix, SLuint32 numIfaces,
  ...
  debugPrintf("OpenSLES: eng_CreateOutputMix called\n");
...
uint32_t slCreateEngine(void **pEngine, uint32_t numOptions, const void *pEngineOptions,
  ...
  debugPrintf("OpenSLES: slCreateEngine called\n");
  if (!g_reg_lock)
    g_reg_lock = SDL_CreateMutex();
  if (!pEngine)
    return SL_RESULT_PARAMETER_INVALID;
