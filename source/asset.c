/* asset.c -- AAssetManager NDK emulation over the loose APK assets
 *
...
// An absolute path: a libnx device path ("sdmc:/...") or leading '/'. the runner only
...
// Absolute root of the loose assets (set from main via asset_set_root); a
// relative root would break as soon as the runner chdir()s.
static char s_root[600] = ASSETS_DIR;

void asset_set_root(const char *dir) {
  if (dir && *dir) snprintf(s_root, sizeof(s_root), "%s", dir);
}
...
static void resolve(char *out, size_t out_size, const char *name) {
  ...
  if (!strncmp(name, "assets/", 7))
    name += 7;
  snprintf(out, out_size, "%s/%s", s_root, name);
}
...
// some engine paths memory-map small assets; materialise the whole file once.
